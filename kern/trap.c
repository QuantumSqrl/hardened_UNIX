...
page_fault_handler(struct Trapframe *tf)
{
	...
	uintptr_t tf_t = UXSTACKTOP - sizeof(struct UTrapframe);

	if(tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp < UXSTACKTOP){
		tf_t = tf->tf_esp - 4 - sizeof(struct UTrapframe);
	}

	user_mem_assert(curenv, (const void*)tf_t, sizeof(struct UTrapframe), PTE_W);
		
	struct UTrapframe* utf = (struct UTrapframe*) tf_t;
