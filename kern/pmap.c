...
boot_alloc(uint32_t n)
{
	...
	if (n) {
		...
		if (nextfree >= (char*)(KADDR(0x7fff000))){
			nextfree = result;
			panic("boot_alloc");
		}
...
mem_init(void)
{
	...
	boot_map_region(kern_pgdir, (uintptr_t) pages, sizeof(struct PageInfo) * npages, PADDR(pages), PTE_W);
...
page_alloc(int alloc_flags)
{
	...
	if (page_free_list != NULL){
		struct PageInfo * new_page = (struct PageInfo*)page_free_list;
		if (!new_page) return NULL;
		page_free_list = page_free_list->pp_link;
		new_page->pp_link = NULL;
		if (alloc_flags & ALLOC_ZERO) memset(page2kva(new_page), 0, PGSIZE);
		return new_page;
	} else {
		return NULL;
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!(pde & PTE_P)) {
		...
		if (!create){
			...
			new_page->pp_ref++;
			pgdir[PDX(va)] = page2pa(new_page) | PTE_W | PTE_P | PTE_U;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for (int i = 0; i < (size); i += PGSIZE){
		...
		if (p_pte == NULL) panic("boot_map_region");
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	if(!pte_store || !p_pte || !(*p_pte & PTE_P)) return NULL;
	*pte_store = p_pte;
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	boot_map_region(kern_pgdir, curr, size, pa, PTE_PCD | PTE_PWT | PTE_W);
	
	return (void*)(curr + (pa & 0xfff));
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (void* i = (ROUNDDOWN((void*)va, PGSIZE)); i < (ROUNDUP((void*)(va+len), PGSIZE)); i += PGSIZE){
		...
		perm = perm | PTE_P;
		if (!page || (*page & perm) != perm || (uintptr_t) i > ULIM){
			user_mem_check_addr = (uintptr_t)(i < va ? va : i);
			return -E_FAULT;
		}
	}
	return 0;
