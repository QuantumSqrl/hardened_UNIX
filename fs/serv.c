...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if((r = openfile_lookup(envid, req->req_fileid, &file)) < 0) return r;
	
	if((r = file_read(file->o_file, ret->ret_buf, req->req_n, file->o_fd->fd_offset)) < 0) return r;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	if ((r = file_write(file->o_file, req->req_buf, req->req_n, file->o_fd->fd_offset)) < 0) return r;
